/*
 * apaleo Booking API
 *
 * Resources and methods to manage guest journeys.
 *
 * API version: v1
 */

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package bookingclient

import (
	"encoding/json"
)

// ReservationServiceModel struct for ReservationServiceModel
type ReservationServiceModel struct {
	Service EmbeddedServiceModel `json:"service"`
	// The date this service is delivered
	ServiceDate string `json:"serviceDate"`
	// The count of booked services
	Count int32 `json:"count"`
	Amount AmountModel `json:"amount"`
	// Whether this service is already booked as extra
	BookedAsExtra bool `json:"bookedAsExtra"`
}

// NewReservationServiceModel instantiates a new ReservationServiceModel object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewReservationServiceModel(service EmbeddedServiceModel, serviceDate string, count int32, amount AmountModel, bookedAsExtra bool) *ReservationServiceModel {
	this := ReservationServiceModel{}
	this.Service = service
	this.ServiceDate = serviceDate
	this.Count = count
	this.Amount = amount
	this.BookedAsExtra = bookedAsExtra
	return &this
}

// NewReservationServiceModelWithDefaults instantiates a new ReservationServiceModel object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewReservationServiceModelWithDefaults() *ReservationServiceModel {
	this := ReservationServiceModel{}
	return &this
}

// GetService returns the Service field value
func (o *ReservationServiceModel) GetService() EmbeddedServiceModel {
	if o == nil {
		var ret EmbeddedServiceModel
		return ret
	}

	return o.Service
}

// GetServiceOk returns a tuple with the Service field value
// and a boolean to check if the value has been set.
func (o *ReservationServiceModel) GetServiceOk() (*EmbeddedServiceModel, bool) {
	if o == nil  {
		return nil, false
	}
	return &o.Service, true
}

// SetService sets field value
func (o *ReservationServiceModel) SetService(v EmbeddedServiceModel) {
	o.Service = v
}

// GetServiceDate returns the ServiceDate field value
func (o *ReservationServiceModel) GetServiceDate() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.ServiceDate
}

// GetServiceDateOk returns a tuple with the ServiceDate field value
// and a boolean to check if the value has been set.
func (o *ReservationServiceModel) GetServiceDateOk() (*string, bool) {
	if o == nil  {
		return nil, false
	}
	return &o.ServiceDate, true
}

// SetServiceDate sets field value
func (o *ReservationServiceModel) SetServiceDate(v string) {
	o.ServiceDate = v
}

// GetCount returns the Count field value
func (o *ReservationServiceModel) GetCount() int32 {
	if o == nil {
		var ret int32
		return ret
	}

	return o.Count
}

// GetCountOk returns a tuple with the Count field value
// and a boolean to check if the value has been set.
func (o *ReservationServiceModel) GetCountOk() (*int32, bool) {
	if o == nil  {
		return nil, false
	}
	return &o.Count, true
}

// SetCount sets field value
func (o *ReservationServiceModel) SetCount(v int32) {
	o.Count = v
}

// GetAmount returns the Amount field value
func (o *ReservationServiceModel) GetAmount() AmountModel {
	if o == nil {
		var ret AmountModel
		return ret
	}

	return o.Amount
}

// GetAmountOk returns a tuple with the Amount field value
// and a boolean to check if the value has been set.
func (o *ReservationServiceModel) GetAmountOk() (*AmountModel, bool) {
	if o == nil  {
		return nil, false
	}
	return &o.Amount, true
}

// SetAmount sets field value
func (o *ReservationServiceModel) SetAmount(v AmountModel) {
	o.Amount = v
}

// GetBookedAsExtra returns the BookedAsExtra field value
func (o *ReservationServiceModel) GetBookedAsExtra() bool {
	if o == nil {
		var ret bool
		return ret
	}

	return o.BookedAsExtra
}

// GetBookedAsExtraOk returns a tuple with the BookedAsExtra field value
// and a boolean to check if the value has been set.
func (o *ReservationServiceModel) GetBookedAsExtraOk() (*bool, bool) {
	if o == nil  {
		return nil, false
	}
	return &o.BookedAsExtra, true
}

// SetBookedAsExtra sets field value
func (o *ReservationServiceModel) SetBookedAsExtra(v bool) {
	o.BookedAsExtra = v
}

func (o ReservationServiceModel) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if true {
		toSerialize["service"] = o.Service
	}
	if true {
		toSerialize["serviceDate"] = o.ServiceDate
	}
	if true {
		toSerialize["count"] = o.Count
	}
	if true {
		toSerialize["amount"] = o.Amount
	}
	if true {
		toSerialize["bookedAsExtra"] = o.BookedAsExtra
	}
	return json.Marshal(toSerialize)
}

type NullableReservationServiceModel struct {
	value *ReservationServiceModel
	isSet bool
}

func (v NullableReservationServiceModel) Get() *ReservationServiceModel {
	return v.value
}

func (v *NullableReservationServiceModel) Set(val *ReservationServiceModel) {
	v.value = val
	v.isSet = true
}

func (v NullableReservationServiceModel) IsSet() bool {
	return v.isSet
}

func (v *NullableReservationServiceModel) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableReservationServiceModel(val *ReservationServiceModel) *NullableReservationServiceModel {
	return &NullableReservationServiceModel{value: val, isSet: true}
}

func (v NullableReservationServiceModel) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableReservationServiceModel) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


